...
var stageWidth=1e3,stageHeight=1e3,stage=new Konva.Stage({container:"container",width:stageWidth,height:stageHeight,draggable:!0}),layer=new Konva.Layer;stage.add(layer);var layerStory=new Konva.Layer;stage.add(layerStory);var el,newPoint,newPlace,offset,layerUser=new Konva.Layer;function fitStageIntoParentContainer(){var e=document.querySelector("#stage-parent").offsetWidth/stageWidth;stage.width(stageWidth*e),stage.height(stageHeight*e),stage.scale({x:e,y:e}),stage.draw()}stage.add(layerUser),window.addEventListener("resize",fitStageIntoParentContainer);var scaleBy=1.01;function test(){Konva.Image.fromURL("/dokdo.svg",function(e){e.setAttrs({width:100,height:50,x:850,y:300}),console.log(e),layer.add(e)})}function scalerInit(){$("#canvasScaler").val(stage.scaleX()),$("#canvasScaler").on("input",function(){var e=$("#canvasScaler").val(),t=stage.scaleX(),a=stage.width()/2/t-stage.x()/t,n=stage.height()/2/t-stage.y()/t;stage.scale({x:e,y:e});var s={x:-(a-stage.width()/2/e)*e,y:-(n-stage.height()/2/e)*e};stage.position(s),stage.batchDraw(),el=$(this),width=el.width(),newPoint=(el.val()-el.attr("min"))/(el.attr("max")-el.attr("min")),console.log(newPoint),offset=-1.7,newPoint<0?newPlace=0:1<newPoint?newPlace=width:(newPlace=width*newPoint+offset,offset-=newPoint);var i=document.querySelector("#stage-parent").offsetWidth/stageWidth;console.log(i),el.next("output").css({left:newPlace,marginLeft:offset+"%"}).text("x"+Math.round(el.val()/i*10)/10)}).trigger("input"),$("#canvasScaler").mouseover(function(){(el=$(this)).next("output").css({display:"inline-block"})}),$("#canvasScaler").mouseleave(function(){(el=$(this)).next("output").css({display:"none"})}),$("#canvasScaler").next("output").css({display:"none"})}stage.on("wheel",function(e){e.evt.preventDefault();var t=stage.scaleX();if(2<t)stage.scale({x:2,y:2});else if(t<.1)stage.scale({x:.1,y:.1});else{var a=stage.getPointerPosition().x/t-stage.x()/t,n=stage.getPointerPosition().y/t-stage.y()/t,s=0<e.evt.deltaY?t*scaleBy:t/scaleBy;stage.scale({x:s,y:s}),$("#canvasScaler").val(s);var i={x:-(a-stage.getPointerPosition().x/s)*s,y:-(n-stage.getPointerPosition().y/s)*s};stage.position(i),stage.batchDraw()}});